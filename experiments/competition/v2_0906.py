# -*- coding: utf-8 -*-
r"""
cnc_three_regimes_physical_v9_blend_ar1.py

重點更新（v9）：
- 變溫段「加權融合」：用上下文小回測 RMSE 的倒數當權重，將 SARIMAX 與 UCM-LLT 的預測做加權平均（無回測時採等權）
- LLT 加入殘差 AR(1)：UnobservedComponents(level="local linear trend", autoregressive=1, exog=...)
- SARIMAX 搜尋網格擴充：加入 (2,1,2)、(3,1,1)、(1,2,1)、(0,1,2)
- 動態特徵：dTC、dTC_pos/neg、cum_dTC_pos/neg、Tnorm、dMotor（自動與 lag 特徵一併生成）
- X 軸白名單擴充：允許上述動態特徵與 PT02（若存在）及其 lag 參與自動特徵挑選（仍有 Top-K 限制）
- 評分：維持「暖機後全區間」計算 RMSE，缺失預測以「上一個預測→否則上一個實際」回填
- 驗證抽樣：預設 RANDOM_SEED=None → 每次執行隨機抽不同的 8 檔；給整數即可固定

建議：
- 先用此版本直接跑；若想再降 RMSE，可對 regime 門檻做小型 grid（見 Config）
"""
...
PRINT_VAR_BLEND = False  # global toggle for per-block blend prints

VERSION = "v9-blend-LLTAR1-dynfeat"
...
class Config:
    ...
    HOLDOUT_K: int = 8
    RANDOM_SEED: Optional[int] = None   # None→每次隨機不同；改成整數可重現
    ...
    DEBUG_RUN_KEYWORD: str = "20200821"

CFG = Config()
# CFG = __apply_safe_overrides__(CFG)  # moved to main()
# ================== 工具 ==================
    ...
    for (b_start, b_end, b_reg) in blocks:
        ...
        if b_reg == "warmup":
            ...
            if has_bt:
                ...
                eps = 1e-6
                eps = 1e-6
                ws = 1.0 / ((rmse_s + eps)**2 + eps)
                wl = 1.0 / ((rmse_l + eps)**2 + eps)
                s = ws + wl
                ws, wl = (ws/s), (wl/s)
                ws = np.clip(ws, 0.1, 0.9)
                wl = 1.0 - ws
# (removed legacy wl=1/(rmse_l+eps); using robust squared-inverse weighting above)
# (replaced by robust squared-inverse weighting with clipping)
...
# ================== 主程式 ==================
# ====== injected safe overrides (do not affect scoring rules) ======
def __apply_safe_overrides__(cfg):
    try:
        import numpy as np
    except Exception:
        pass
    try:
        ...
        pass
    try:
        cfg.LAGS = [1,2,3,5,8,13,21]
    except Exception:
        pass
    try:
        cfg.CONTEXT_STEPS = 240
    except Exception:
        pass
    try:
        cfg.BACKTEST_B = 80
        ...
        pass
    try:
        cfg.MIN_KEEP_RATIO = 0.80
    except Exception:
        pass
    for k,v in [('AUTO_EXOG_TOPK_X',24),('AUTO_EXOG_TOPK_Z',18),('MAX_WARM_PRED_STEPS',120)]:
        try:
            setattr(cfg,k,v)
        except Exception:
            pass
    # WARM_K_GRID as logspace if possible
    try:
        import numpy as np
        setattr(cfg, 'WARM_K_GRID', list(np.logspace(-5, 0.3, 60)))
    except Exception:
        pass
    return cfg
# ===== end injected =====
...
def main(cfg: Config = CFG):
    cfg = __apply_safe_overrides__(cfg)
    ...
    tr_df, va_df, _ = zscore_fit_transform(tr_df, va_df, exog_candidates_all)
    ...
    for target in cfg.TARGET_COLS:
        ...
        for run_name, g in va_df.groupby("run_id"):
            ...
            if cfg.DEBUG_RUN_KEYWORD and (cfg.DEBUG_RUN_KEYWORD in run_name):
                ...
                for r in ["warmup","const","variable"]:
                    m = (g["regime"].values == r) & eval_mask
                    if m.any():
                        rrmse = float(np.sqrt(np.nanmean((yhat_full[m] - g[target].values[m])**2)))
                        print(f"[DEBUG/RMSE] {target} {run_name} {r}: {rrmse:.4f} (n={m.sum()})")
                if X_run is not None:
                    miss = g[exog_cols_t].isna().mean().sort_values(ascending=False)
                    miss.to_csv(os.path.join("debug", f"missing_exog_{run_name.replace('.csv','')}_{target}.csv"),
                                header=["missing_rate"])
