...
class Config:
    ...
    # 來自 env.xlsx 的自訂白名單（可為空）
    WHITE_X: List[str] = field(default_factory=list)
    WHITE_Z: List[str] = field(default_factory=list)
...
def add_derived_features(df: pd.DataFrame, win: int = 60) -> pd.DataFrame:
    ...
    out["t_idx"] = out.groupby("run_id").cumcount().astype(float)
    if "TC_slope" in out.columns:
        out["TC_accel"] = out["TC_slope"].diff()
    return out
...
def _with_lags(bases: List[str], lags: List[int], pool: List[str]) -> List[str]:
    wanted = set()
    for base in bases:
        wanted.add(base)
        for L in lags:
            wanted.add(f"{base}_lag{L}")
    # 僅回傳實際存在於 pool 的欄位
    return [c for c in pool if c in wanted]
...
# ================== env.xlsx 載入 ==================
def load_env_excel(cfg: Config, env_path: str):
    """從 Excel 讀取 paths/params/holdout/lags/white_x/white_z 並覆寫到 cfg"""
    try:
        xls = pd.ExcelFile(env_path)
    except Exception as e:
        print(f"[ENV] 無法讀取 {env_path}：{e}")
        return cfg

    def _sheet(name):
        return pd.read_excel(xls, sheet_name=name) if name in xls.sheet_names else None

    # paths: key,value
    df = _sheet("paths")
    if df is not None and {"key","value"}.issubset(set(map(str.lower, df.columns.str.lower()))):
        cols = {c.lower(): c for c in df.columns}
        kv = dict(zip(df[cols["key"]], df[cols["value"]]))
        if "train" in kv: cfg.TRAIN_DIR = str(kv["train"])
        if "test"  in kv: cfg.TEST_DIR  = str(kv["test"])
        if "out"   in kv: cfg.SUBMISSION_DIR = str(kv["out"])
        print(f"[ENV] paths 覆寫完成：train={cfg.TRAIN_DIR} | test={cfg.TEST_DIR} | out={cfg.SUBMISSION_DIR}")

    # params: key,value 覆寫 Config 同名屬性
    df = _sheet("params")
    if df is not None and {"key","value"}.issubset(set(map(str.lower, df.columns.str.lower()))):
        cols = {c.lower(): c for c in df.columns}
        for k, v in zip(df[cols["key"]], df[cols["value"]]):
            if not hasattr(cfg, str(k)): 
                continue
            # 嘗試轉成數值/布林
            sval = str(v)
            if sval.lower() in {"true","false"}:
                ...
                try:
                    val = float(sval) if "." in sval or "e" in sval.lower() else int(sval)
                except Exception:
                    val = v
            setattr(cfg, str(k), val)
        print("[ENV] params 已覆寫。")

    # holdout: filename 單欄
    df = _sheet("holdout")
    if df is not None:
        col = df.columns[0]
        cfg.HOLDOUT_FILES = [str(x) for x in df[col].dropna().astype(str).tolist()]
        print(f"[ENV] holdout 檔數：{len(cfg.HOLDOUT_FILES)}")
    ...
    if df is not None:
        col = df.columns[0]
        try:
            cfg.LAGS = [int(x) for x in df[col].dropna().astype(int).tolist()]
            print(f"[ENV] lags = {cfg.LAGS}")
        except Exception as e:
            print(f"[ENV] 解析 lags 失敗：{e}")

    # white lists：white_x / white_z
    dx = _sheet("white_x")
    if dx is not None:
        cfg.WHITE_X = [str(x) for x in dx[dx.columns[0]].dropna().astype(str).tolist()]
        print(f"[ENV] white_x 讀入 {len(cfg.WHITE_X)} 欄：{cfg.WHITE_X}")
    dz = _sheet("white_z")
    if dz is not None:
        cfg.WHITE_Z = [str(x) for x in dz[dz.columns[0]].dropna().astype(str).tolist()]
        print(f"[ENV] white_z 讀入 {len(cfg.WHITE_Z)} 欄：{cfg.WHITE_Z}")
...
def _anchor_continuity(yhat_seg: np.ndarray, y_series: pd.Series, start_pred: int) -> np.ndarray:
    if yhat_seg is None or len(yhat_seg) == 0:
        return yhat_seg
    if start_pred > 0 and np.isfinite(y_series.iloc[start_pred - 1]):
        delta = float(y_series.iloc[start_pred - 1]) - float(yhat_seg[0])
        return yhat_seg + delta
    return yhat_seg

def _cap_jumps(yhat_seg: np.ndarray, y_series: pd.Series, start_pred: int,
               lookback: int = 200, k: float = 4.0) -> np.ndarray:
    """限制預測段的跳幅：用最近 lookback 真值增量的 std 來當上限"""
    if yhat_seg is None or len(yhat_seg) == 0:
        return yhat_seg
    lb = max(1, start_pred - lookback)
    diffs = y_series.iloc[lb:start_pred].diff().dropna().values
    if len(diffs) == 0:
        return yhat_seg
    thr = float(np.std(diffs)) * k
    if not np.isfinite(thr) or thr <= 0:
        return yhat_seg

    capped = yhat_seg.copy()
    # 第一跳：和上一個實際值相比
    prev = float(y_series.iloc[start_pred - 1]) if start_pred > 0 else float(y_series.iloc[0])
    first = np.clip(capped[0] - prev, -thr, thr) + prev
    shift = first - capped[0]
    capped[0] = first
    if len(capped) > 1:
        # 後續也限制單步跳幅（相對前一預測）
        for i in range(1, len(capped)):
            step = capped[i] - capped[i-1]
            if step > thr: capped[i] = capped[i-1] + thr
            elif step < -thr: capped[i] = capped[i-1] - thr
    return capped
...
def predict_run_three_models(res_const, res_var_smx, res_var_ll,
    ...
    for (b_start, b_end, b_reg) in blocks:
        ...
        if X is not None:
            X_ctx = X.iloc[ctx_start:b_end+1].copy()
            X_ctx = X_ctx.clip(-8, 8)   # ★ 把標準化後的特徵限制在 [-8, 8]
        else:
            X_ctx = None
        ...
        if b_reg == "warmup":
            ...
            if len(y_prefix) >= 5:
                ...
                yhat_w = _anchor_continuity(yhat_w, y, start_pred)   # ← 新增
                yhat_w = _cap_jumps(yhat_w, y, start_pred)
                Lw = len(yhat_w)
                if Lw > 0:
            ...
            if rest_start <= b_end:
                ...
                if (rest_start - ctx_start) < n_eff:
                    ...
                    yhat_c = _anchor_continuity(yhat_c, y, rest_start)   # ← 新增
                    yhat_c = _cap_jumps(yhat_c, y, rest_start)
                    Lc = len(yhat_c)
            ...
            if rel_start_ctx < n_eff:
                ...
                yhat_seg = _anchor_continuity(yhat_seg, y, start_pred)   # ★ 新增：把段落第一點對齊前一點
                L = len(yhat_seg)
            ...
            if rel_start_ctx < n_eff:
                ...
                yhat_seg = _anchor_continuity(yhat_seg, y, start_pred)  # ← 新增
                yhat_seg = _cap_jumps(yhat_seg, y, start_pred)
                L = len(yhat_seg)
                yhat[start_pred:start_pred+L] = yhat_seg
                eval_mask[start_pred:start_pred+L] = True
...
def main(cfg: Config = CFG):
    ...
    for target in cfg.TARGET_COLS:
        ...
        if target == "Disp. X":
            ...
            force_keep += _with_lags(phys_keep, cfg.LAGS, exog_candidates_all)
            
        elif target == "Disp. Z":
            phys_keep_z = [
                "Tnorm","TC_mean","TC_slope","TC_vol","dTC",
                "Spindle Motor","Motor_sum","dMotor",
                "PT_mean","PT02","TC07"
            ]
            force_keep += _with_lags(phys_keep_z, cfg.LAGS, exog_candidates_all)

        # env.xlsx 的自訂白名單（X/Z 皆可）
        env_keep = (cfg.WHITE_X if target == "Disp. X" else cfg.WHITE_Z)
        if env_keep:
            force_keep += _with_lags(env_keep, cfg.LAGS, exog_candidates_all)

        # 去重
        force_keep = list(dict.fromkeys(force_keep))

        # 用相關性挑剩下的名額，與 force_keep 合併後截到 topk
        picked = auto_exog_for_target(tr_df, exog_candidates_all, target, topk, priority_keep=[])
        exog_cols_t = list(dict.fromkeys(force_keep + picked))[:topk]
        print(f"[INFO] 強制白名單後 EXOG = {len(exog_cols_t)} 欄：{exog_cols_t}")
        # ★ Step 1a: 先把訓練特徵清乾淨（ffill/bfill + 剔除高缺值）
        ...
        # ★ Step 1b: 再保險一次，把任何殘留的 NaN/Inf 補掉
        if X_c is not None: X_c = clean_exog_apply(X_c)
        if X_v is not None: X_v = clean_exog_apply(X_v)

        # 最後保險檢查
        for name, Xchk in [("X_const", X_c), ("X_var", X_v)]:
            if Xchk is not None and (np.isinf(Xchk.values).any() or np.isnan(Xchk.values).any()):
                bad_cols = Xchk.columns[Xchk.isna().any() | ~np.isfinite(Xchk).all()].tolist()
                print(f"[DEBUG] {name} 有 NaN/Inf 的欄位（前 10 個）：{bad_cols[:10]}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--env", default="", help="環境設定 Excel（如 env.xlsx）")
    args = p.parse_args()
    ...
    if args.env:
        CFG = load_env_excel(CFG, args.env)
    main(CFG)