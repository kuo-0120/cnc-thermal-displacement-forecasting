...
def main(cfg: Config = CFG):
    ...
    for target in cfg.TARGET_COLS:
        ...
        priority_base = ["TC_mean","TC_slope","TC_vol","Motor_sum","dMotor","PT_mean","dTC","Tnorm"]
        # 追加 env.xlsx 的白名單
        priority_base += (cfg.WHITE_X if target == "Disp. X" else cfg.WHITE_Z)
        priority = [c for c in priority_base if c in exog_candidates_all]
        ...
        # ★ env.xlsx 自訂白名單（X/Z 皆可）：將白名單 + 其 lag 視為候選，若有則優先以相關性挑滿 topk
        env_keep = (cfg.WHITE_X if target == "Disp. X" else cfg.WHITE_Z)
        if env_keep:
            env_keep_with_lags = set(env_keep)
            for base in env_keep:
                for L in cfg.LAGS:
                    env_keep_with_lags.add(f"{base}_lag{L}")
            candidates_env = [c for c in exog_candidates_all if c in env_keep_with_lags]
            if candidates_env:
                wl2 = auto_exog_for_target(tr_df, candidates_env, target, min(topk, len(candidates_env)), priority_keep=[])
                # 以白名單結果優先；若與原選集不同，取並集（避免過度收斂）
                if wl2:
                    exog_cols_t = list(dict.fromkeys(wl2 + exog_cols_t))
                    print(f"[INFO] env.xlsx 白名單({target}) 套用後 EXOG = {len(exog_cols_t)} 欄：{exog_cols_t}")
